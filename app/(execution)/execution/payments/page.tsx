...
const SOURCE_LABELS: Record<string, string> = {
  INBOUND: "Purchase (Gate In)",
  OUTBOUND: "Sale (Gate Out)",
  SPOT: "Purchase (Spot)",
};
...
export default function FinanceQueuePage() {
  ...
  const resolved = useMemo(() => (payments ?? []).filter((p) => p.status !== "PENDING"), [payments]);
  const pendingPagination = useListPagination(pending);
  const resolvedPagination = useListPagination(resolved);
  ...
  return (
    <div className="kastros-desk-page">
      <div>
        ...
        <h1 className="mt-2 text-2xl font-bold text-foreground">Finance Queue</h1>
                ...
                <div key={p.id} className="p-5 space-y-3">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-mono text-sm font-bold text-warning">{p.tradeRef}</span>
                      ...
                      <p className="mt-1 text-sm font-medium text-foreground">{p.counterpartyName}</p>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <input placeholder="Finance comment (optional)"
                      ...
                      className="flex-1 min-w-[200px] rounded-lg px-3 py-1.5 text-xs text-foreground"
      ...
      {resolved.length > 0 && (
        <div className="rounded-2xl" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
          ...
          <div className="divide-y" style={{ borderColor: "rgba(255,255,255,0.04)" }}>
            {resolvedPagination.items.map((p) => {
              const color = SOURCE_COLORS[p.sourceType] ?? "#6b7280";
              ...
              return (
                <div key={p.id} className="flex flex-wrap items-center justify-between gap-3 px-5 py-3">
                  <div className="flex flex-wrap items-center gap-3 text-xs">
                    <span className="font-mono font-semibold text-warning">{p.tradeRef}</span>
          ...
          <ListPagination
            ...
            startIndex={resolvedPagination.startIndex}
            endIndex={resolvedPagination.endIndex}
