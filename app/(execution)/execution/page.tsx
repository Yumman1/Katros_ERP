...
function contractsHref(profile?: string, scope?: string, commodity?: string) {
...
export default function ExecutionDeskPage() {
  ...
  const commodityOptions = useMemo(() => {
    ...
    for (const t of pending ?? []) map.set(t.commodityCode, t.commodityCode);
    return [...map.entries()]
  ...
  const filteredPending = useMemo(() => {
    if (commodityFilter === "ALL") return pending ?? [];
    return (pending ?? []).filter((t) => t.commodityCode === commodityFilter);
  }, [pending, commodityFilter]);
  ...
  const pdOpen = openContracts.filter((c) => c.executionProfile === "PURCHASE_DELIVERED");
  const spotOpen = openContracts.filter((c) => c.executionProfile === "PURCHASE_SPOT");
  const saleOpen = openContracts.filter((c) => c.executionProfile === "SALE_EX_WAREHOUSE");

  const kpis = [
    {
      label: "Locked Open",
      value: openContracts.length,
      sub: `${filteredContracts.length} total locked`,
    ...
    },
    {
      ...
      value: pdOpen.length,
      sub: "Open contracts",
      href: contractsHref("PURCHASE_DELIVERED", undefined, commodityFilter),
      color: "#60a5fa",
    ...
    {
      label: "Purchase Spot",
      value: spotOpen.length,
      sub: "Open contracts",
      href: contractsHref("PURCHASE_SPOT", undefined, commodityFilter),
      color: "#818cf8",
    ...
    {
      label: "Sales",
      value: saleOpen.length,
      sub: "Ex-warehouse",
      href: contractsHref("SALE_EX_WAREHOUSE", undefined, commodityFilter),
      color: "#a78bfa",
  ...
  const recentContracts = filteredContracts.slice(0, 8);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href={contractsHref(undefined, undefined, commodityFilter)}
        ...
        <div className="rounded-2xl p-4" style={{ background: "rgba(245,158,11,0.06)", border: "1px solid rgba(245,158,11,0.2)" }}>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-amber-400">
              {filteredPending.length} trade{filteredPending.length !== 1 ? "s" : ""} awaiting lock
            </h2>
          </div>
          ...
          <ul className="ml-6 mt-2 space-y-1">
            {filteredPending.slice(0, 5).map((t) => (
          ...
          <Link href={contractsHref(undefined, undefined, commodityFilter)} className="ml-6 mt-2 inline-flex items-center gap-1 text-xs font-medium text-amber-400 hover:underline">
            View all →
          </Link>
        </div>
      )}
      ...
      <div className="rounded-2xl" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
        <div className="flex items-center justify-between border-b px-5 py-4" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
          ...
          <Link href={contractsHref(undefined, undefined, commodityFilter)} className="text-xs font-medium text-amber-400 hover:underline">View all</Link>
        </div>
        <div className="overflow-x-auto">
          ...
          {recentContracts.length === 0 && (
            <div className="flex flex-col items-center justify-center py-12 text-zinc-500">
              ...
              <p className="text-sm">No locked contracts for this commodity yet.</p>
              <p className="text-xs mt-1 text-zinc-600">Try another commodity or ask trader to lock more contracts.</p>
            </div>
          )}
        </div>
      </div>
