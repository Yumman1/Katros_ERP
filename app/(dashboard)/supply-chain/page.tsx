...
const severityColor = {
  ...
  low: "text-muted-foreground border-zinc-500/30 bg-zinc-500/10",
};
...
export default function SupplyChainPage() {
  ...
  if (isLoading || !overview) {
    return <div className="animate-pulse text-subtle">Loading supply chain operations…</div>;
  }
  ...
  return (
    <div className="kastros-desk-page">
      <div>
        <h1 className="text-2xl font-semibold text-foreground">Supply Chain Operations</h1>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6">
        {[
          ...
          <div key={t.label} className="rounded-lg border border-kastros-border bg-kastros-card px-3 py-2.5">
            ...
            <div className="mt-1 data-grid text-base font-medium text-foreground">{t.value}</div>
          </div>
        ))}
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          <div key={t.label} className="rounded-lg border border-kastros-border bg-kastros-card px-3 py-2.5">
            <div className="text-xs uppercase tracking-wide text-subtle">{t.label}</div>
            <div className={`mt-1 text-sm ${"warn" in t && t.warn ? "text-red-400" : "text-foreground"}`}>{t.value}</div>
          </div>
        ))}
      </div>

      <div className="rounded-lg border border-kastros-border bg-kastros-card p-4">
        <h2 className="text-sm font-medium text-muted-foreground">Physical flow pipeline</h2>
        <p className="mt-1 text-xs text-subtle">From supplier confirmation through to customer delivery</p>
              ...
              <div
                className={`rounded-md border px-3 py-2 text-xs ${
                  stage.status === "complete"
                    ...
                    : "border-kastros-border bg-kastros-bg text-muted-foreground"
                }`}
              >
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="lg:col-span-2 rounded-lg border border-kastros-border bg-kastros-card p-4">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-medium text-muted-foreground">Location utilization</h2>
            <Link href="/locations" className="text-xs text-success hover:underline">
              View all locations →
            </Link>
          </div>
        ...
        <div className="rounded-lg border border-kastros-border bg-kastros-card p-4">
          <h2 className="text-sm font-medium text-muted-foreground">Operational alerts</h2>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="overflow-hidden rounded-lg border border-kastros-border bg-kastros-card">
          <div className="flex items-center justify-between border-b border-kastros-border px-3 py-2">
            ...
            <Link href="/shipments" className="text-xs text-success hover:underline">
              Full logistics view →
            </Link>
          </div>
        ...
        <div className="overflow-hidden rounded-lg border border-kastros-border bg-kastros-card">
          <div className="flex items-center justify-between border-b border-kastros-border px-3 py-2">
            <span className="text-sm text-muted-foreground">Position vs physical inventory</span>
            <Link href="/reconciliation" className="text-xs text-success hover:underline">
              Reconciliation →
            </Link>
          </div>
          <div className="max-h-72 overflow-auto text-sm">
              ...
              <tbody>
                {posInv?.map((r) => (
                  <tr key={r.code} className="border-b border-kastros-border/60">
                    ...
                    <td className={`px-2 py-1.5 data-grid ${r.variance !== 0 ? "text-warning" : ""}`}>
      ...
      <div className="flex flex-wrap gap-2">
        <Link href="/inventory" className="rounded-md border border-kastros-border px-3 py-1.5 text-sm text-success hover:bg-foreground/5">
          Inventory lots & movements
        </Link>
        <Link href="/shipments" className="rounded-md border border-kastros-border px-3 py-1.5 text-sm text-success hover:bg-foreground/5">
          Logistics & shipments
        </Link>
        <Link href="/locations" className="rounded-md border border-kastros-border px-3 py-1.5 text-sm text-success hover:bg-foreground/5">
          Warehouse network
        </Link>
        <Link href="/suppliers" className="rounded-md border border-kastros-border px-3 py-1.5 text-sm text-success hover:bg-foreground/5">
          Supplier performance
        </Link>
        <Link href="/traceability" className="rounded-md border border-kastros-border px-3 py-1.5 text-sm text-success hover:bg-foreground/5">
          Batch traceability
        </Link>
      </div>
    </div>
