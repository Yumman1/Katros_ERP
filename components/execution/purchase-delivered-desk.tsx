...
export function PurchaseDeliveredDesk({ scope }: { scope: TradeScope }) {
  ...
  const { data: contracts, isLoading: loadingContracts } = trpc.execution.lockedContracts.useQuery(
    { profile: "PURCHASE_DELIVERED", tradeScope: scope },
  ...
  return (
    <div className="space-y-6">
      <TradeScopeHeader
        ...
        title="Gate In — Delivered Purchases"
        subtitle="Trucks from the warehouse gate · assign to open delivered purchase orders"
      />
      ...
      <div className="grid grid-cols-3 gap-3">
        <StatChip
          ...
          color="#f59e0b"
        />
        <StatChip label="Open purchase orders" value={filteredContracts.length} color="#34d399" />
        <StatChip
          label="Open volume"
