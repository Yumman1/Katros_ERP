...
export function FinanceDoApprovalsPanel() {
  ...
  const { data: rows, isLoading, error } = trpc.finance.doApprovals.useQuery(undefined, {
  ...
  return (
    <div className="kastros-desk-page">
      <PageHeader
        breadcrumb={
          <>
            <Link href="/finance/approvals/payments" className="hover:text-foreground">
              Finance
            </Link>
            <span>/</span>
      ...
      <div className="kastros-desk-scroll">
        {approve.error && <p role="alert" className="mb-3 text-sm text-destructive">{approve.error.message}</p>}
        {error ? <p role="alert" className="text-sm text-destructive">Unable to load delivery orders: {error.message}</p> : isLoading ? (
