...
export const TRADE_CONTRACT_PRINT_CSS = `
...
/** The contract note body — one template for trader and execution print/preview. */
export function TradeContractPrintDocument({ trade }: { trade: MockTraderTrade }) {
  const quotedUnit =
  ...
  const unitPrice = trade.pricePerCanonicalQty ?? trade.price;
  const goodsValue = trade.quantity * unitPrice;
  const commission = trade.commissionAmount ?? 0;
  const params_ = (trade.tradeParams ?? {}) as Record<string, string | number | null>;
  const tolerance =
    formatQuantityTolerance(params_, trade.quantityUnit) ??
  ...
  return (
    <div className="print-doc mx-auto my-6 w-full max-w-[720px] border border-black bg-white p-8">
      <div className="border-b-2 border-black pb-3 text-center">
        ...
        <div className="mt-3 text-lg font-bold uppercase tracking-[0.2em]">{contractTitle}</div>
      </div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-x-6 gap-y-3 border-t border-black/30 pt-4">
        ...
        <Field label="Commission" value={commission ? fmtMoney(commission, trade.currency) : "—"} />
          ...
          <tbody>
            <tr>
              <td className="py-1 text-neutral-600">
                Goods value ({fmtQty(trade.quantity)} {trade.quantityUnit} × {fmtQty(unitPrice)}{" "}
                {trade.currency}/{trade.quantityUnit})
              </td>
      ...
      <p className="mt-6 border-t border-black/20 pt-2 text-center text-[10px] leading-relaxed text-neutral-600">
        ...
        <span className="font-mono font-semibold">{trade.tradeRef}</span>. If it is not signed and
        returned, and no written objection is received, within 24 hours of issue, it shall be deemed
        accepted and remains valid and binding on both parties without signature.
      </p>
...
export function TradeContractPrintView({
  trade,
  isLoading,
  error,
  backHref,
  ...
  trade: MockTraderTrade | null | undefined;
  isLoading?: boolean;
  error?: string | null;
  backHref: string;
  backLabel: string;
  ...
  const contractTitle =
    trade?.direction === "BUY" ? "Purchase Contract" : trade?.direction === "SELL" ? "Sale Contract" : "Contract";

  return (
    <div className="min-h-screen bg-white text-black">
      <style>{TRADE_CONTRACT_PRINT_CSS}</style>
      <div className="no-print flex flex-wrap items-center gap-2 border-b border-black/10 px-4 py-3">
        ...
        {secondaryHref && secondaryLabel ? (
          <Link href={secondaryHref} className="text-sm text-neutral-600 underline">
            {secondaryLabel}
          </Link>
        ) : null}
        {trade && (
          <span className="text-sm text-neutral-600">
            {contractTitle} · {trade.tradeRef}
          </span>
        )}
        <button
          type="button"
      ...
      {error && <p className="p-8 text-sm text-red-700">{error}</p>}
      {trade && <TradeContractPrintDocument trade={trade} />}
    </div>
  );
}
